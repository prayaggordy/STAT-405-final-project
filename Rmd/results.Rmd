...
### Are there clear differences in the rates of full vaccinations between different regions?
```{r}
plot_vax_by_region()
```
The histogram above was obtained by combining census data at a county level with the county-level vaccination data from the CDC and state sources. We used the variable "fully_vax" as the data for the histogram and faceted by the census region (South, West, Midwest, Northeast) of the county.
...
grid.arrange(nrow=2, ncol=2,
						 grobs=list(region_plots$fully_vax$midwest,
						 					 region_plots$fully_vax$northeast,
						 					 region_plots$fully_vax$west,
						 					 region_plots$fully_vax$south))
```
We created these maps using ggplot's sf geometry, the county-level total population census data, total case data, and total full vaccination data.

These maps show a county-level breakdown of the proportion of the population that is fully vaccinated. Once again, it is clear that there are regions of the country, especially the South (even more specifically, Georgia), that are severely under-vaccinated. In fact, only the Northeast region is reaching higher levels of vaccination, and while there are pockets of high vaccination in the West and Midwest, such as in the Seattle area, those regions do not have a uniformly high vaccination rate thus far. Perhaps most alarming is that despite many counties reaching a 45% to 65% full vaccination rate, only a few select counties have crossed the 65% full vaccination threshold thus far. This is important, as many scientists place the threshold for herd immunity in a community at around 65-70% full vaccination, suggesting that as a country, we are quite far from reaching a safe vaccination level.

```{r}
library(gridExtra); library(grid); library(lattice)
region_plots <- plot_regions()
grid.arrange(nrow=2, ncol=2,
						 grobs=list(region_plots$cases_fully_vax$midwest,
						 					 region_plots$cases_fully_vax$northeast,
						 					 region_plots$cases_fully_vax$west,
						 					 region_plots$cases_fully_vax$south))
```
These maps are colored on a gradient based on the ratio of cases per 100k to the full vaccination percentage. In order to do this, we created a cases/100k variable using a combination of case and census data and then a ratio variable using the cases/100k and the fully vaccinated proportion.

We believed that this variable would be interesting in that it highlights the counties with either high case rates, low vaccination rate, or both. 
