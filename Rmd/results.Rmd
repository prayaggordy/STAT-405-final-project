...
```{r}
tk <- race_hist_plot()
#comparisons <- c("Low Vax Republican vs. Low Vax Democrat", "High Vax Democrat vs. Low Vax Democrat", "High Vax Republican vs. Low Vax Democrat",
#								 "High Vax Democrat vs. Low Vax Republican", "High Vax Republican vs. Low Vax Republican", "High Vax Republican vs. High Vax Democrat")
data.frame(tk$orig_df) %>% mutate(p.adj = round(p.adj, 4), diff = round(diff, 4), lwr = round(lwr, 4), upr = round(upr, 4)) -> tk_df
colnames(tk_df) <- c("Difference in Mean Proportion Black", "95% Confidence Lower Bound", "95% Confidence Upper Bound", "p-value")
